...
import 'package:event_planner/repositories/planner_profile_repository.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';

class PlannerProfileScreen extends StatefulWidget {
  const PlannerProfileScreen({super.key});

  @override
  State<PlannerProfileScreen> createState() => _PlannerProfileScreenState();
}

class _PlannerProfileScreenState extends State<PlannerProfileScreen> {
  ...
  void initState() {
    super.initState();
    PlannerProfileRepository.profile.addListener(_onProfileChanged);

    final cachedProfile = PlannerProfileRepository.cachedProfile;
    if (cachedProfile != null) {
      _applyProfile(cachedProfile);
      _isLoading = false;
      unawaited(PlannerProfileRepository.refreshInBackground());
    } else {
  ...
  Future<void> _loadProfile({bool forceRefresh = true}) async {
    final hasCache = PlannerProfileRepository.hasCache;
    ...
    try {
      final profile = await PlannerProfileRepository.loadProfile(
  ...
  Future<void> _savePersonalInfo() async {
    ...
    try {
      final updatedProfile = await PlannerProfileRepository.updateProfile(
  ...
  Future<void> _updatePassword() async {
    ...
    try {
      await PlannerProfileRepository.updatePassword(
  ...
  @override
  void dispose() {
    PlannerProfileRepository.profile.removeListener(_onProfileChanged);
